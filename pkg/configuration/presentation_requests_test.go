...
claim_mappings:
  a: "b"
`
	if err := os.WriteFile(filepath.Join(dir, "t1.yaml"), []byte(content), 0644); err != nil {
		t.Fatal(err)
	}
...
claim_mappings:
  a: "b"
`
	os.WriteFile(filepath.Join(dir, "t1.yaml"), []byte(strings.Replace(tmpl, "%s", "s1", 1)), 0644)
	os.WriteFile(filepath.Join(dir, "t2.yaml"), []byte(strings.Replace(tmpl, "%s", "s2", 1)), 0644)
...
claim_mappings:
  a: "b"
`
	os.WriteFile(filepath.Join(dir, "t1.yaml"), []byte(t1), 0644)
	os.WriteFile(filepath.Join(dir, "t2.yaml"), []byte(t2), 0644)
